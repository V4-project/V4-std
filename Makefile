...
build:
	@echo "🔨 Building..."
	@cmake -B build -DCMAKE_BUILD_TYPE=Debug -DV4STD_BUILD_TESTS=ON
	@cmake --build build -j

# Release build
release:
	@echo "🚀 Building release..."
	@cmake -B build-release -DCMAKE_BUILD_TYPE=Release -DV4STD_BUILD_TESTS=ON
...
asan: clean
	...
	@cmake -B build-asan -DCMAKE_BUILD_TYPE=Debug -DV4STD_BUILD_TESTS=ON \
...
ubsan: clean
	...
	@cmake -B build-ubsan -DCMAKE_BUILD_TYPE=Debug -DV4STD_BUILD_TESTS=ON \
...
size:
	...
	@echo ""
	@echo "=== Library ==="
	@if [ -f build-release/libv4std.a ]; then \
		cp build-release/libv4std.a build-release/libv4std.stripped.a && \
		strip --strip-debug build-release/libv4std.stripped.a && \
		ls -lh build-release/libv4std.a build-release/libv4std.stripped.a | awk '{print $$9 ": " $$5}'; \
	fi
	@echo ""
